...
class Staff extends AdminController
{
    ...
    public function index()
    {
        ...
        /*  $data['usr'] = $this->load->model('Staff_model');
            if($this->equipe == 'Equipe IARD Sylvia'){ */ 
        $data['staff_members'] = $this->staff_model->get('', ['active' => 1]);
        $this->load->model('Staff_model','staff');
        $this->load->model('Equipe_model','equipe');
        $data['row'] = $this->equipe->getc(); 
        $id = get_staff_user_id();
        $data['id'] = $id;
        $data['equipeid'] = $this->staff->get($id);
    ...
    public function Path($user_id = '')
    {
        ...
        $data['row'] = $this->equipe->getc();  */ 
        //$this->load->model('Staff_model','staff');
        //$data['row1'] = $this->staff->getSt();
        $data['Equipe'] = $this->staff->getStaff($user_id);
                
        $arraystaff = $data['Equipe'];
        $tabassign =  array();
        foreach ($arraystaff as $idstaff) {
            $id = $idstaff->staffid;
            $data['assign'] = $this->leads->get_assigned($id);
            $assign = $data['assign'] ;
            
            $tabassign[$id] = $assign;
            
            $data['tabassign']=$tabassign;
            
            
        }
        $data['title']         = _l('Equipe');
      
        $this->load->view('admin/staff/manage2', $data);
    ...
    {
        $this->load->model('Staff_model','staff');
         $data['user'] = $this->staff->getStaff();
        
         
        //$data['title'] = 'Ma Page';
         //$this->load->view('admin/Equipe/Equipe_view',$data);
         $this->load->view('admin/staff/member',$data);
    } 
