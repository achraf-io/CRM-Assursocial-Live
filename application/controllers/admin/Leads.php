...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        $data['all_leads'] = $this->leads_model->getAll();
        ...
        $data['leadid'] = $id;
        $current_id = get_staff_user_id();
        $data['current_id'] = $current_id;
         
        
        $data['staff_1'] = $this->staff_model->get('', ['equipe' => "Equipe IARD Sylvia"]);
        
        $data['staff_2'] = $this->staff_model->get('', ['equipe' => "Equipe Santé Marina"]);
        $data['staff_resp'] = $this->staff_model->get('', ['role' => 1]);
        $this->load->model('Leads_model', 'staff');
        $data['equipe'] = $this->staff->getEq();
        
      
            /*  $data['leads_by_assigned'] = $this->leads_model->getAll_by_assigned();
             array(); */
           // $leads_by_assigned =$data['leads_by_assigned'];
        

        // afficher les prospect pour le responsable d'equipe;
    ...
    public function bulk_action()
    {
        ...
        if ($this->input->post()) {
            ...
            $service = $this->input->post('nom_service');
            if (is_array($ids)) {
                foreach ($ids as $id) {
