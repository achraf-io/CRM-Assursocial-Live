...
class Leads_model extends App_Model
{
    ...
    public function getEq()
    {
        ...
        $query =  $this->db->query('SELECT staffid FROM tblstaff where equipe = "Equipe IARD Sylvia" ');
    ...
    public function getAll_by_assigned()
    {
    ...
    public function get_assigned1($id)
    {
        $this->db->select('assigned');
        $this->db->from('leads');
        $this->db->where('assigned',$id);
    ...
    public function add($data)
    {
        ...
        // ajout de service
       // $data['nom_service'] = trim($data['nom_service']);
