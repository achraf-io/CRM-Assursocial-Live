...
class Staff_model extends App_Model
{
    ...
    /* public function getStaff($user_id){
        // $this->db->where('id',$user_id);
         $this->db->select('*');
         $this->db->from('staff');
         $this->db->join('equipe', 'id = staff.equipe_id');
        // $this->db->limit(3);
       $this->db->where('id',$user_id);
         $query = $this->db->get();
         //print_r($query);
...
public function get_Staff_equipe()
{
    ...
    public function getStaff($user_id)
    {
         ...
         $this->db->select('*');
         $this->db->from('staff');
         $this->db->join('equipe', 'Equipe_name = staff.equipe');
     ...
     public function get_Staff()
    {
     ...
     public function getStaff_equipe($user_id)
     {
     ...
     public function getStaff_role($user_id)
     {
        $query = $this->db->query("SELECT role FROM tblstaff  WHERE staffid = $user_id");
        $row = $query->row_array();
        return $row;
     ...
     public function getS()
    {
        // $this->db->where('id',$user_id);
         ...
         $this->db->from('staff');
         //$this->db->join('equipe', 'Equipe_name = staff.equipe');
         //$this->db->join('equipe', 'staff.equipe_id = id');
        // $this->db->limit(3);
        //$this->db->where('staffid',17);
         $query = $this->db->get();
         //print_r($query);
         return $query->result();
     ...
     {
        $query = $this->db->query('SELECT staffid FROM tblstaff LIMIT 1');
        $row1 = $query->row_array();
        return $row1;
